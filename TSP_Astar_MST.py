...
# Get arguments
filename = sys.argv[1]
if DEBUG: print "Read file from" + filename
...
T = G.nodes()
list.sort(T)
g = [-1 for i in range(len(T))] # g[]: the path length from root node to the specific node in the graph G. Initial values are -1.
...
while heap:
    ...
    if visit_status[int(current_node)] == -1:
        ...
        if DEBUG: print str(count) + "th loop: node " + current_node + ", visited " + str(g[int(current_node)]) + ", estimate total path " + str(estimate_min_path)
        ...
        for neighbor in neighbors:
            ...
            if visit_status[neighbor_index] == -1: 
                ...
                if DEBUG_verbose:
                    print "g:" + str(g[neighbor_index]) + "\t h:" + str(h) + "\tf:" +str(f[neighbor_index])
                parent[neighbor_index] = current_node
                heapq.heappush(heap, [f[neighbor_index], neighbor])
    else:
        if(DEBUG): print "error (from DEBUG): found visited node" + current_node

# Return back to the source node
parent[int(T[0])] = current_node
g[int(T[0])] = g[int(current_node)] + G[current_node][T[0]]['weight'] #g[0] store the path from the last unvisited node to source node
...
while node_counter>0:
    ...
    node_counter -= 1
if DEBUG: print path_array

# output results
print "Tour: " + ''.join(path_array)
print "Cost: " + str(g[int(T[0])])